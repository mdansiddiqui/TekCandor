...
namespace NohaFMS.Services
{
    public class ReportService : BaseService, IReportService
    {
        ...
        public virtual PagedResult<MyReport> GetMyReports(string expression,
            dynamic parameters,
            int pageIndex = 0,
            int pageSize = 2147483647,
            IEnumerable<Sort> sort = null)
        {
            var searchBuilder = new SqlBuilder();
            var search = searchBuilder.AddTemplate(SqlTemplate.MyReportSearch(), new { skip = pageIndex * pageSize, take = pageSize });
            if (!string.IsNullOrEmpty(expression))
                searchBuilder.Where(expression, parameters);
            if (sort != null)
            {
            ...
            var countBuilder = new SqlBuilder();
            var count = countBuilder.AddTemplate(SqlTemplate.MyReportSearchCount());
            if (!string.IsNullOrEmpty(expression))
                countBuilder.Where(expression, parameters);

            using (var connection = _dapperContext.GetOpenConnection())
            {
                var reports = connection.Query<MyReport>(search.RawSql, search.Parameters);
                var totalCount = connection.Query<int>(count.RawSql, search.Parameters).Single();
                return new PagedResult<MyReport>(reports, totalCount);
            }
        }
        ...
        public virtual Stream CrystalReportExport(Report report, Dictionary<string, string> dictSearch, List<IEnumerable<dynamic>> data, int exportFormatType, string reportSecondTitle, User currentUser, Site clientInfo)
        {
