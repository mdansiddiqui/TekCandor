...
namespace NohaFMS.Services
{
    ...
    public class SendMessageJob : IJob
    {
            ...
            foreach (var message in messages)
            {
                try
                {
                    if (message.MessageType == (int?)MessageType.Email)
                    {
                        ...
                        List<NohaFMS.Core.Domain.Attachment> messageAttachments = new List<Core.Domain.Attachment>();
        ...
        public void SendEmailMessage(string senderEmail, string ccRecipientEmails, string recipientEmails, string header, string message, List<NohaFMS.Core.Domain.Attachment> messageAttachments)
        {
            ...
            if (messageAttachments != null)
                foreach (NohaFMS.Core.Domain.Attachment messageAttachment in messageAttachments)
                {
        ...
        private void SendAppleNotification(Message message)
        {
            ...
            // Sandbox
            //var file = new X509Certificate2(File.ReadAllBytes("NohaFMS_push_sandbox.p12"), "Ng11235813$",
            //    X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
            //var config = new ApnsConfiguration(ApnsConfiguration.ApnsServerEnvironment.Sandbox,
            //    "NohaFMS_push_sandbox.p12", "Ng11235813$");

            // Production
            var file = new X509Certificate2(File.ReadAllBytes("NohaFMS_push_production.p12"), "Ng112358$",
                X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet | X509KeyStorageFlags.Exportable);
            var config = new ApnsConfiguration(ApnsConfiguration.ApnsServerEnvironment.Production,
                "NohaFMS_push_production.p12", "Ng112358$");
