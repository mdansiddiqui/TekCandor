...
namespace NohaFMS.Services
{
    ...
    public partial class CurrencyService : BaseService, ICurrencyService
    {
        ...
        /// {0} : currency ID
        /// </remarks>
        private const string CURRENCIES_BY_ID_KEY = "NohaFMS.currency.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        private const string CURRENCIES_ALL_KEY = "NohaFMS.currency.all";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string CURRENCIES_PATTERN_KEY = "NohaFMS.currency.";
        ...
        public virtual decimal? GetLiveExchangeRate(long currencyId)
        {
            ...
            if (primaryExchangeCurrency == null)
                throw new NohaFMSException("Primary exchange rate currency is not set");

            var currency = _currencyRepository.GetById(currencyId);
            if (currency == null)
                throw new NohaFMSException("Currency is not exist.");
        ...
        public virtual decimal ConvertToPrimaryExchangeRateCurrency(decimal amount, Currency sourceCurrencyCode)
        {
            ...
            if (result != decimal.Zero && sourceCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new NohaFMSException(string.Format("Exchange rate not found for currency [{0}]", sourceCurrencyCode.Name));
        ...
        public virtual decimal ConvertFromPrimaryExchangeRateCurrency(decimal amount, Currency targetCurrencyCode)
        {
            ...
            if (result != decimal.Zero && targetCurrencyCode.Id != primaryExchangeRateCurrency.Id)
            {
                ...
                if (exchangeRate == decimal.Zero)
                    throw new NohaFMSException(string.Format("Exchange rate not found for currency [{0}]", targetCurrencyCode.Name));
                result = result * exchangeRate;
            }
            return result;
        }
