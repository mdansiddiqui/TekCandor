...
namespace NohaFMS.Services
{
    ...
    public partial class UserRegistrationService : IUserRegistrationService
    {
        ...
        public virtual UserLoginResults ValidateUser(string loginName, string password)
        {
            ...
            if (user.PasswordLastChanged.Value.AddDays(30) < DateTime.Now)
            {
                user.IsPasswordChangeRequired = true;
                return UserLoginResults.PasswordExpire;
            }
            else user.IsPasswordChangeRequired = false;
            ...
            if (!isValid)
            {
                ...
                if (user.FailedLoginAttempts > 3 && user.IsDeleted == false)
                {
                    user.Active = false;
                    return UserLoginResults.UserBlocked;
