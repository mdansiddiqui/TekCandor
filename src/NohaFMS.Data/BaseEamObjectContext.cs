...
using NohaFMS.Core.Timing;
using NohaFMS.Data.Mapping;
...
namespace NohaFMS.Data
{
    ...
    /// Object context
    /// </summary>
    public class NohaFMSObjectContext : DbContext, IDbContext
    {
        #region Ctor

        //This is used for EF migrations
        //TODO - get connection strings dynamically and setup for deploying on production
        public NohaFMSObjectContext()
            : base("NohaFMS")

        {
        }

        public NohaFMSObjectContext(string nameOrConnectionString)
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            .Where(type => type.IsAbstract != true && type.BaseType != null && type.BaseType.IsGenericType &&
                (type.BaseType.GetGenericTypeDefinition() == typeof(NohaFMSEntityTypeConfiguration<>)
                    || type.BaseType.GetGenericTypeDefinition() == typeof(NohaFMSWorkflowEntityTypeConfiguration<>)));
        ...
        public override int SaveChanges()
        {
            ...
            {
                var fail = new NohaFMSException("Unable to save changes. The record you attempted to edit was modified by another user after you got the original value.");
                throw fail;
            }
        ...
        public TEntity GetById<TEntity>(object id, bool useRawSql) where TEntity : BaseEntity
        {
            ...
            if (useRawSql == true)
            {
                var dnAttribute = typeof(TEntity).GetCustomAttributes(typeof(TableAttribute), true).FirstOrDefault() as TableAttribute;
                string type = string.Empty;
                if (dnAttribute != null)
                {
                    type = dnAttribute.Name;
                }
        ...
        public object GetByEntityIdAndType(long entityId, string entityType)
        {
            ...
            Type type = asm.GetType("NohaFMS.Core.Domain." + entityType);

            //user reflection to call generic method
            MethodInfo method = typeof(NohaFMSObjectContext).GetMethod("GetById");
