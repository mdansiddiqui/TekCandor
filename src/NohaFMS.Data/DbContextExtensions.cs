...
namespace NohaFMS.Data
{
    public static class DbContextExtensions
    {
        ...
        public static List<string> GetColumnNames(this IDbContext context, string entityType, string param)
        {
            var result = new List<string>();
            Type entType = Type.GetType(entityType);
            var adapter = ((IObjectContextAdapter)context).ObjectContext;
            ...
            var q = from meta in metadataWorkspace.GetItems(DataSpace.CSpace).Where(m => m.BuiltInTypeKind == BuiltInTypeKind.EntityType)
                    ...
                    .Where(p => p.DeclaringType.Name == entityType && p.Name.IndexOf(param, StringComparison.CurrentCultureIgnoreCase) != -1)
                    select p;

            result = q.Select(e => e.Name).ToList();
            return result;
        }
