...
namespace NohaFMS.Core.WebApi.Security
{
    public class HmacAuthentication
    {
        private static readonly string _delimiterRepresentation = "\n";
        private static readonly string _scheme = "NohaFMSHmac";
        ...
        public bool CreateKeys(out string key1, out string key2, int length = 32)
        {
            ...
            using (var rng = RandomNumberGenerator.Create())
            {
                for (int i = 0; i < 9999; ++i)
                {
                    ...
                    key1 = ToHexString(data1, length).ToLower();
                    key2 = ToHexString(data2, length).ToLower();
        ...
        public bool ParseTimestamp(string timestamp, out DateTime time)
        {
            ...
            if (DateTime.TryParseExact(timestamp, "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out time))
                return true;
		...
		private string ToHexString(byte[] bytes, int length = 0)
        {
