...
namespace NohaFMS.Web.Framework.CustomField
{
    ...
    public partial class FieldModel
    {
        ...
        public string ToExpression(int index)
        {
            ...
            //updating DateFilter expression on all below operators to call MySql Date function and trim-off time part
            else if (this.Operator == FieldOperatorType.eq)
            {
                ...
                else if (this.ControlType == FieldControlType.Date && this.DataType == FieldDataType.DateTimeNullable)
                {
                    expression = string.Format("DATE({0}) = @i{1}", this.DbColumn, index);
                }
                else
                {
            ...
            else if (this.Operator == FieldOperatorType.neq)
            {
                ...
                if (this.ControlType == FieldControlType.Date && this.DataType == FieldDataType.DateTimeNullable)
                {
                    expression = string.Format("DATE({0}) != @i{1}", this.DbColumn, index);
                }
                else
