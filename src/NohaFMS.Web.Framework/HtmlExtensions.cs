...
using NohaFMS.Web.Framework.Localization;
...
namespace NohaFMS.Web.Framework
{
    public static class HtmlExtensions
    {
            ...
            string action, string controller, RouteValueDictionary routeValues = null, bool required = false, string dataBind = "", bool aysIgnore = false)
        {
            ...
            {
                HtmlId = id,
                Area = routeValues != null && routeValues.ContainsKey("area") ? routeValues["area"].ToString() : "",
        ...
        //public static MvcHtmlString SubmitButton<TModel,TValue>(this HtmlHelper helper, string name, string text, object htmlAttributes = null)
        //{
        ...
        public static MvcHtmlString BaseEamMultiSelectBoxFor<TModel, TValue>(this HtmlHelper<TModel> helper,
            Expression<Func<TModel, TValue>> expression,
            string action, string controller, RouteValueDictionary routeValues = null, bool required = false, string dataBind = "", bool aysIgnore = false, bool multiple = false)
        {
            var result = new StringBuilder();
            //if (aysIgnore == true)
            //    result.Append(helper.TextBoxFor(expression, new { data_bind = dataBind, style = "display: none;", @class = "ays-ignore" }));
            ...
            //    result.Append(helper.TextBoxFor(expression, new { data_bind = dataBind, style = "display: none;" }));
            var id = helper.FieldIdFor(expression);
            var dropdown = helper.Partial("KendoMultiSelect", new BaseEamComboBox
            {
                HtmlId = id,
                Controller = controller,
                Action = action,
        ...
        public static MvcHtmlString BaseEamFieldValue(this HtmlHelper helper, FieldModel model)
        {
            ...
            else if (model.ControlType == FieldControlType.DropDownList)
            {
                ...
                else if (model.DataSource == FieldDataSource.Enum)
                    result.Append(helper.Partial("FieldComboBoxEnum", model));
            }
