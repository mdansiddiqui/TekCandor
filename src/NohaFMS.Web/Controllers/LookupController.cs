...
namespace NohaFMS.Web.Controllers
{
    public class LookupController : BaseController
    {
        ...
        public int OrdinalIgnoreCase { get; private set; }
        ...
        public ActionResult AssetList(DataSourceRequest command, string searchValues, IEnumerable<Sort> sort = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                PagedResult<NohaFMS.Core.Domain.Asset> data = _assetService.GetAssets(model.ToExpression(), model.ToParameters(), command.Page - 1, command.PageSize, sort);
        ...
        public ActionResult LocationList(DataSourceRequest command, string searchValues, IEnumerable<Sort> sort = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                PagedResult<NohaFMS.Core.Domain.Location> data = _locationService.GetLocations(model.ToExpression(), model.ToParameters(), command.Page - 1, command.PageSize, sort);
        ...
        public ActionResult MeterEventList(long? assetId, long? locationId, string meterName, DataSourceRequest command, IEnumerable<Sort> sort = null)
        {
            if (ModelState.IsValid)
            {
              ...
              .ToList();
                ...
                {
                    meterEvents = meterEvents.Where(m => m.Meter.Name.IndexOf(meterName, OrdinalIgnoreCase) >= 0).ToList();
                }
