...
namespace NohaFMS.Web.Controllers
{
    public class VisualController : BaseController
    {
        ...
        {
            //UserActivityLog
            _userActivityService.InsertActivityLog("UrlAccessed", _httpContext.Request.RawUrl, "Visual List");
            var model = _httpContext.Session[SessionKey.VisualSearchModel] as SearchModel;
            if (model == null)
                model = BuildSearchModel();
            else
            ...
            if (ModelState.IsValid)
            {
                ...
                //SearchFilters
                var fVisual = model.Filters.ElementAt(0).Value as string;
                _visualService.searchFilterLog(fVisual);
                _httpContext.Session[SessionKey.VisualSearchModel] = model;
