...
namespace NohaFMS.Web.Controllers
{
    public class TaskGroupController : BaseController
    {
        ...
        {
            //UserActitvitLog
            _taskGroupService.MaintenanceActivityFlowlog(_workContext.CurrentUser.Id,"User["+_workContext.CurrentUser.Name+"] Maintenance Task Groups List");
            _userActivityService.InsertActivityLog("UrlAccessed", _httpContext.Request.RawUrl, "Task Group");
            var model = _httpContext.Session[SessionKey.TaskGroupSearchModel] as SearchModel;
            if (model == null)
                model = BuildSearchModel();
            else
