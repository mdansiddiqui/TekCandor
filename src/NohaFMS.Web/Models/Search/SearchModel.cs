...
namespace NohaFMS.Web.Models
{
    ...
    public class SearchModel
    {
        ...
        //updating DateFilter date format according to the database Date Format
        public dynamic ToParameters()
        {
            ...
            if (count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (filter.ControlType == FieldControlType.Date && filter.DataType == FieldDataType.DateTimeNullable)
                    {
                      parameters.Add("i" + i,  ((DateTime)this.NoEmptyFilters[i].Value).ToString("yyyy-MM-dd"));
                    }
                    else
                    {
            ...
            foreach (var filter in this.Filters)
            {
                ...
                if (dict.ContainsKey(filter.Name) && !string.IsNullOrEmpty(dict[filter.Name]))
                {
                    ...
                    if (filter.DataType == FieldDataType.Int32)
                        ...
                        filter.Value = (DateTime?)Convert.ToDateTime(dict[filter.Name]);
